...
# Setup the Window
win = visual.Window(
    size=[1820, 980], fullscr=True, screen=0, 
...
# Initialize components for Routine "pasteating"
pasteatingClock = core.Clock()
text_8 = visual.TextStim(win=win, name='text_8',
    text="Have you eaten or drank anything other than water in the last 3 hours?\n\n\nPress 'y' for Yes and 'n' for No.",
    font='Open Sans',
    pos=(0, 0), height=0.035, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
# ------Prepare to start Routine "pasteating"-------
continueRoutine = True
# update component parameters for each repeat
key_resp_7.keys = []
key_resp_7.rt = []
_key_resp_7_allKeys = []
# keep track of which components have finished
pasteatingComponents = [text_8, key_resp_7]
for thisComponent in pasteatingComponents:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
pasteatingClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "pasteating"-------
while continueRoutine:
    # get current time
    t = pasteatingClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=pasteatingClock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    ...
    if key_resp_7.status == STARTED and not waitOnFlip:
        theseKeys = key_resp_7.getKeys(keyList=['y', 'n', 'Y', 'N'], waitRelease=False)
...
thisExp.nextEntry()
# the Routine "pasteating" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()
...
trials = data.TrialHandler(nReps=1.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('C:\\Users\\ACLab\\Desktop\\Ratatouille Study\\imageList.xlsx', selection='0:228'),
...
for thisTrial in trials:
    ...
    _key_resp_6_allKeys = []
    # keep track of which components have finished
    attentionComponents = [image_2, key_resp_6, text_7]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if key_resp_6.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > key_resp_6.tStartRefresh + 2.5-frameTolerance:
                # keep track of stop time/frame for later
